...
3. Check the platform's alert-management area for an existing alert matching your target role/location first (same check as Step 2c) — only create a new one at the alert creation area (per the table) if none exists, sent to the email address configured in `CLAUDE.md`.
