...
Gmail is the only email connector available today — this isn't a preference over Outlook or any other provider, there's simply nothing else to connect yet. The single-mailbox safety gate is the part that matters and would apply the same way if another email connector becomes available later.
