...
Gmail is the one built-in, fully automated path — genuinely the easy option. Using Outlook instead? Anthropic's official [Claude for Outlook](https://support.claude.com/en/articles/14855664-use-claude-for-outlook) add-in can't power this repo's automation (no MCP/tool-call interface, and it's oriented around a Microsoft 365 work/school account — check Microsoft AppSource for the current word on personal-account support). A real alternative exists as an optional, self-directed advanced path (a self-hosted MCP server, Azure App Registration required) — see [SETUP.md](SETUP.md)'s advanced Outlook section and `.claude/commands/outlook-sync.md`, currently an unverified draft. Whichever mailbox you use, keep it to one — see `CLAUDE.md`'s Account Restriction section.
