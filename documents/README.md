...
│       ├── application_packet.md # Generated by /apply — contact fields, screening-question
│       │                        # answers, and cover letter body for Claude in Chrome / manual form-filling
...
**`application_packet.md`** — Generated automatically by `/apply` (Step 7), not something you write by hand. Bundles contact fields, the canonical screening-question answers (work rights, visa status — see `04-job-evaluation.md`), the CV/cover-letter PDF paths, and a plain-text cover letter body, all in one place — meant to be handed to the **Claude in Chrome** browser extension (or used for manual form-filling) so filling out the actual application form takes minimal re-typing. It deliberately never includes referee names (see the References section in `01-candidate-profile.md`) and always tells whoever's filling the form to stop before clicking submit — final submission is a human action, not something `/apply` automates. `/setup` does not read this file.
...
Application folders may also contain **`interview_prep_<stage>.md`** files written by `/interview` (one per interview stage, kept as history) and **`application_packet.md`** written by `/apply`. `/setup` reads only `job_posting.md`, `cover_letter.tex`, `cv_draft.tex`, and `outcome.md`, and ignores these.
